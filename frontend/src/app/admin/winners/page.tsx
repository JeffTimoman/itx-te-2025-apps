...
export default function WinnersPage() {
  ...
  // Combined filter: toolbar + per-column header
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      // Toolbar search across gift/category/winner fields
      ...
      // Header: category filter (text, independent of dropdown)
      const catTxt = (r.category_name || "").toLowerCase();
      const cfOk = !cf || catTxt.includes(cf);

      // Header: winners filter (name/code + assigned status)
      ...
      const anyWinnerMatch =
        r.winners && r.winners.length > 0
          ? r.winners.some((w) => {
              ...
              const textOk = !wf || text.includes(wf);
              return textOk && assignedOk(w);
            })
          : // No winners: keep only if winner text filter is empty and assignedFilter isn't strictly "assigned"
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      let res = 0;
      if (sortKey === "gift") {
        res = cmpStr(a.gift_name, b.gift_name);
      } else if (sortKey === "category") {
        // Null/empty categories at the bottom
        res = cmpStr(a.category_name || "~~~", b.category_name || "~~~");
      } else if (sortKey === "winners") {
        const ca = a.winners?.length ?? 0;
        const cb = b.winners?.length ?? 0;
        res = ca - cb;
      }
  ...
  // Sort toggle helper
  function toggleSort(nextKey: SortKey, canSort: boolean) {
    if (!canSort) return;
    setSortKey((prevKey) => {
      if (prevKey !== nextKey) {
        setSortDir("asc");
          ...
          <div className="overflow-auto">
            <table className="w-full text-sm">
              <thead className="bg-white/10 sticky top-0 z-10">
                ...
                {/* Filter inputs row (inside the same thead so it stays sticky) */}
