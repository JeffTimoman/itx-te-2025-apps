...
type Food = { id: number; name: string; created_at?: string | null };
...
export default function FoodsPage() {
  ...
  async function createFood(e?: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text());
      const created = (await res.json()) as Food;
      setFoods((f) => [created, ...f]);
      setNewName("");
    } catch (err) {
      setError(toMessage(err));
    } finally {
  ...
  async function saveEdit() {
    ...
    setError(null);
    try {
      const res = await authFetch(`/api/admin/foods/${editing.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name }),
      });
  ...
  async function deleteFood(id: number) {
    ...
    setError(null);
    try {
      const res = await authFetch(`/api/admin/foods/${id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error(await res.text());
  ...
  const baseFiltered = useMemo(() => {
    ...
    return foods.filter((f) =>
      `${f.id}|${f.name}|${f.created_at || ""}`.toLowerCase().includes(q)
    );
  }, [foods, query]);
  ...
  const filtered = useMemo(() => {
    ...
    return baseFiltered.filter((f) => {
      const idOk = !idf || String(f.id).toLowerCase().includes(idf);
      const nameOk = !nf || (f.name || "").toLowerCase().includes(nf);
      const createdOk =
        !cf ||
        String(f.created_at || "")
          .toLowerCase()
                ...
                {loading ? (
                  ...
                  paged.map((f) => (
                    ...
                    >
                      ...
                      <td className="p-3 min-w-[16rem]">{f.name}</td>
